...
Eigen::MatrixXcd get_Dirac_matrix ( const double Mu=1.0 ){ //
  ...
  for(int x=0; x<Lx; x++){
    for(int y=0; y<Ly; y++){
      res.block<2,2>(2*idx(x,y), 2*idx(x,y)) = Mu*sigma[0];
    }
  }
...
Eigen::MatrixXcd get_large_epsilon (){
  ...
  for(int x=0; x<Lx; x++){
    for(int y=0; y<Ly; y++){
      res.block<2,2>(2*idx(x,y), 2*idx(x,y)) = I*sigma[2];
    }
  }
...
#pragma omp parallel for collapse(2)
  ...
  for(int x=0; x<Lx; x++){
    for(int y=0; y<Ly; y++){
      const Idx idx1 = 2*idx(x,y);
      res.segment(idx1,2) += v.segment(idx1,2);
    }
  }
...
#pragma omp parallel for collapse(2)
  ...
  for(int y=0; y<Ly; y++) for(int x=0; x<Lx; x++) {
      ...
      for(int mu : {0,1,2} ){
        ...
        // std::cout << "debug. x= " << x << ", y = " << y << ", mu = " << mu
        //           << ", idx1 = "<< idx1 << ", idx2 = " << idx2 << ", vec = " << res.segment(idx1, 2).transpose() << std::endl;
      }
    }
...
#pragma omp parallel for collapse(2)
  ...
  for(int y=0; y<Ly; y++) for(int x=0; x<Lx; x++) {
      ...
      for(int mu : {0,1,2} ){
        ...
        const Idx idx1 = 2*idx(x,y);
        const Idx idx2 = 2*idx(xp,yp);
        res.segment(idx1, 2) -= sign * kappa[mu%3] * Wilson_projector( (mu+3)%6 ) * v.segment(idx2, 2);
      }
    }
